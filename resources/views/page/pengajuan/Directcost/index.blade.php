        ...
        <div class="card p-3 mb-3">
            <div class="d-flex flex-wrap gap-3 justify-content-between align-items-center">
                ...
                <div class="d-flex gap-2">
                    <a href="{{ url('/directp/create') }}" class="btn btn-primary rounded-pill">Tambah</a>
                    <a href="{{ route('directp.export', request()->all()) }}"
            ...
            {{-- IMPORT --}}
            <hr>
            <form action="{{ route('directp.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row g-2 align-items-center">
                    ...
                    <div class="col-md-4">
                        <small>📄 <a href="{{ asset('template_directp_import.xlsx') }}">Download Template</a></small>
                    </div>
                </div>
            </form>
        </div>
                            ...
                            <tbody>
                                @foreach ($data as $d)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $d->Qty }}</td>
                                        <td>{{ $d->Unit }}</td>
                                        <td>{{ $d->Needed_by ?? ($d->workorder->kode_wo ?? '-') }}</td>
                                        ...
                                        <td>
                                            <div class="d-flex gap-2">
                                                @if ($selisih <= 7)
                                                    <a href="{{ route('page.pengajuan.Directcost.edit', $d->id) }}"
                                                        class="btn btn-warning btn-sm">Edit</a>
                                                @else
                                                    <span class="badge bg-secondary">Expired</span>
                                                @endif
                                                <form class="form-soft-delete"
                                                    action="{{ route('page.Directcost.destroy', $d->id) }}" method="POST">
                                                    @csrf @method('DELETE')
                        ...
                        <div class="card mb-3 shadow-sm">
                            <div class="card-body">
                                ...
                                <div class="small">
                                    ...
                                    <div class="d-flex justify-content-between">
                                        <span>Kebutuhan</span>
                                        <span>{{ $d->Needed_by ?? ($d->workorder->kode_wo ?? '-') }}</span>
                                    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form class="form-restore d-inline"
                                            action="{{ route('page.Directcost.restore', $d->id) }}" method="POST">
                                            @csrf
                                            <button class="btn btn-success btn-sm">Restore</button>
                                        </form>
                                        <form class="form-force-delete d-inline"
                                            action="{{ route('page.Directcost.forceDelete', $d->id) }}" method="POST">
                                            @csrf @method('DELETE')
